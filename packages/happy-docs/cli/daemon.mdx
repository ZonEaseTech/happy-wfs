...
When you update the Happy CLI (`npm update -g happy-ai-cli`), the daemon detects the version mismatch on its next heartbeat and automatically restarts with the new version. No manual restart needed.
