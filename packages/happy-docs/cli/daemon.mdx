...
When you update the Happy CLI (`npm update -g @zonease/happy`), the daemon detects the version mismatch on its next heartbeat and automatically restarts with the new version. No manual restart needed.
...
5. Stores its state at `~/.happy-ai/daemon.state.json`
