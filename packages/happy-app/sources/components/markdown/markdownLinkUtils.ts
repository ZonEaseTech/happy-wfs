...
import { isPreviewableHtml, isPreviewableImage, isPreviewableMarkdown, isPreviewableVideo, isTemporaryFilePath } from '@/utils/fileViewer';
...
export function buildSessionFileHref(args: {
    ...
    const queryParams = [
        `path=${encodedPath}`,
        isPreviewableHtml(args.filePath) || isPreviewableVideo(args.filePath) || isPreviewableMarkdown(args.filePath) ? 'view=preview' : 'view=file',
    ];
