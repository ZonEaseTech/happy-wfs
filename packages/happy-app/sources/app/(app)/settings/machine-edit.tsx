...
export default function MachineEditScreen() {
    ...
    const rawPath = searchParams.path as string;
    const validateJson = searchParams.validateJson === '1';
    // path query param is plain (not base64) here so that callers can construct it
    // straightforwardly with encodeURIComponent — see claude-config.tsx
    ...
    React.useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                const response = await machineReadFile(machineId!, filePath);
                if (cancelled) return;
    ...
    const handleSave = React.useCallback(async () => {
        ...
        // Validate JSON before saving when caller asked for it (e.g. settings.json).
        // Bad JSON in ~/.claude/settings.json silently disables every Claude Code
        // setting from that file, so a pre-save check is much friendlier than
        // "everything broke and you have no idea why".
        if (validateJson) {
            try {
                JSON.parse(content);
            } catch (e) {
                Modal.alert(
                    t('common.error'),
                    `${t('machineEdit.invalidJson')}: ${e instanceof Error ? e.message : 'parse error'}`,
                );
                return;
            }
        }
    ...
    }, [content, hasChanges, isSaving, originalHash, machineId, filePath, validateJson]);
