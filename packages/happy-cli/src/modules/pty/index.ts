...
function isBashShell(shell: string): boolean {
    return basename(shell).includes('bash');
}

function createHappyBashRc(): string {
    const dir = mkdtempSync(join(tmpdir(), 'happy-pty-'));
    const rcPath = join(dir, 'bashrc');
    const script = [
        '# Generated by Happy AI for interactive PTY sessions.',
        "# Source the user's normal bash config first, then replace only the prompt.",
        ...
        '  PS1="$(__happy_short_pwd)\\$ "',
        '}',
        '',
        'if [[ -n "${PROMPT_COMMAND:-}" ]]; then',
        '  PROMPT_COMMAND="${PROMPT_COMMAND%;}; __happy_prompt_command"',
        'else',
        "  PROMPT_COMMAND='__happy_prompt_command'",
        'fi',
        '',
    ].join('\n');
    writeFileSync(rcPath, script, 'utf8');
    return rcPath;
}
