...
describe.skipIf(!await isServerHealthy())('Daemon Integration Tests', { timeout: 20_000 }, () => {
   ...
   * 
   * This simulates what happens during `npm upgrade happy-ai-cli`:
   * - Running daemon has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/happy-ai-cli/ with NEW version files
  ...
  // TODO: Test npm uninstall scenario - daemon should gracefully handle when happy-ai-cli is uninstalled
